        ...
        if field is not None:
            ...
            if key == 27:
                ...
                for object_ in robot_coords:
                    if object_["object_class"] in [1, 2, 3]:
                        state = robot.robot.getl()
                        state[0] = object_["centroid_coord"][0]
                        state[1] = object_["centroid_coord"][1]
                        state[2] = 0.15
                        robot.robot_go(state)
                        state_2 = robot.robot.getl()
                        state_2[2] = 0.02
                        ...
                        state_2[2] = 0.2
                        robot.robot.movel(state_2, acc=0.2, vel=0.3)
                        if object_["object_class"] == 3:
                            robot.go_pos_file("positions/green_box", acc=0.2, vel=0.3)
                        elif object_["object_class"] == 1:
                            robot.go_pos_file("positions/orange_box", acc=0.2, vel=0.3)
                        elif object_["object_class"] == 2:
                            robot.go_pos_file("positions/yeelow_box", acc=0.2, vel=0.3)

                        robot.home_state()
                    elif object_["object_class"] in [0, 4]:
                        print("Cube")
                        pass
